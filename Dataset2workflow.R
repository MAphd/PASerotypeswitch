...
p1 <- p1 + ggnewscale::new_scale_fill() +
  geom_nodepoint(size = 5, shape = 21, color = "black", stroke = 0.3, aes(subset = (!is.na(SND))&SND>100,alpha = 1, fill = log(SND) )) +  # geom_nodepoint(size = 0.7, stroke = 0.01, shape = 21, color = "black", fill = NA, aes(subset = (!is.na(SND))&SND>10 )) +
