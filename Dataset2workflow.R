...
#Examples using dataset 2: 

Metadata <- readRDS(paste0(Usedir, "/Metadata/Dataset2meta.Rdata"))
Treefile <- paste0(Usedir, "/Metadata/Dataset2parsnp.tree")
...
p1 <- ggtree(Mytree2, size = 0.2, layout="circular",branch.length = "none") +
  geom_tiplab(size=2.5, aes(x = 41.5, label = ST), check_overlap = TRUE) 

#Annotate with serotype
p1 <- gheatmap(p1, Myserotype, offset = .0, width = 0.05, colnames = FALSE, color = "Black") +
  labs(fill = "Serotype")

#Annotate with serotype switch events
p1 <- p1 + ggnewscale::new_scale_fill() +
  geom_nodepoint(size = 5, shape = 21, color = "black", stroke = 0.3, aes(subset = (!is.na(SND))&SND>Threshold,alpha = 1, fill = (SND) )) +  # geom_nodepoint(size = 0.7, stroke = 0.01, shape = 21, color = "black", fill = NA, aes(subset = (!is.na(SND))&SND>10 )) +
  scale_fill_viridis(alpha=0.8,option = "D", direction = -1,name="ln SND") +
